...
const { mockInvoke, mockUpdate, mockUpdateEq } = supabaseMocks;
...
describe('Index pipeline navigation', () => {
  ...
  it('saves before moving forward with the Next button and allows moving back', async () => {
    ...
    expect(mockUpdateEq).toHaveBeenCalledWith('id', 'new-pipeline-id');
  ...
  it('creates a fresh pipeline after restarting configuration', async () => {
    ...
    const [exportProgressButton] = await screen.findAllByRole('button', { name: /export/i });
    await user.click(exportProgressButton);
    ...
    const userInfoButtonAgain = await screen.findByRole('button', { name: /mock user info form/i });
    await user.click(userInfoButtonAgain);

    const [exportProgressButtonAgain] = await screen.findAllByRole('button', { name: /export/i });
    await user.click(exportProgressButtonAgain);
